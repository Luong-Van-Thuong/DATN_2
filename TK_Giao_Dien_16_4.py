...
def cham_cong():
    ...
    luuHayKhongLuu = 0
    startX, startY, endX, endY = 0, 0, 0, 0  # Kh?i t?o các bi?n này
    while True:
        ret, frame = cap.read()
        if not ret:
            break
            ...
            if confidence > 0.5:
                ...
                if face is not None and not face.size == 0:
                    ...
                    if 10 < conf < 50:
                        ...
                        cv2.putText(frame, "Không biết", (startX, startY + h + 30), font, 0.4, (0, 255, 0), 1)
                else:
                    cv2.putText(frame, "Không phát hiện được khuôn mặt", (10, 30), font, 0.7, (0, 0, 255), 2)
            else:
                cv2.putText(frame, "Không phải người", (startX, startY + h + 30), font, 0.4, (0, 255, 0), 1)  
